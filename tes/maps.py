...
from .base import lookup, Detection, Timing, Height, Signal, TraceType
from . import mca
    ...
    def _get_response(self):
        ...
        if resp != 0:
            ...
            if len(l[:-3]) == 0:
                return
            data = np.frombuffer(
                bytearray.fromhex(l[:-3].decode('utf8')),
                np.uint32).byteswap()[0]
            return data
...
class RegInfo:
    ...
    def _get_reg(self, address, field):
        ...
        if len(field):
            ...
            _logger.debug(
                '_get_reg:extract field{:} -> 0x{:X}'
                .format(_field_str(field), data)
            )
    ...
    def set(self, values, transform=None, indices=None):
        _logger.debug(
            'RegInfo.set({})[{}] to {}'.format(self.name, indices, values)
        )

        if isinstance(values, dict):
            if not self.loadable:
                _logger.debug(
                    'RegInfo.set({}):not loadable'.format(self.name)
                )
                return
        ...
        if transform:
            ...
            _logger.debug(
                'RegInfo.set({})[{}] to {}'
                .format(self.name, indices, reg_values)
            )
    ...
    def _set_reg(self, value, address, field):
        ...
        _logger.debug(
            '_set_reg:{} to {}'.format(_map_str(field, address), value)
        )
        ...
        if field:
            ...
            _logger.debug(
                '_set_reg:0x{:08X} from 0x{:08X} to 0x{:08X}'
                .format(address, old_value, new_value)
            )
        else:
            new_value = int(value)
            _logger.debug(
                '_set_reg:0x{:X} to 0x{:08X}'
...
global_map = OrderedDict([
    ...
    ('adc_enable',
     RegInfo(
         0x10000080, (0x000000FF, 0), False, None, None, True,
         doc="""
        unsigned:8 bit
    ...
    ('event_enable',
     RegInfo(
         0x10000100, (0x000000FF, 0), False, None, None, True,
         doc="""
        unsigned:8 bit
    ...
    ('mtu',
     RegInfo(
         0x10000010, (), False, lambda x: int(x*8), lambda x: x//8, True,
         doc="""
...
mca_map = OrderedDict([
    ('lowest_value',
     RegInfo(
        0x10000004, (), False, lambda x: int(np.int32(x)),
        lambda x: int(np.uint32(x)), True,
        doc="""
        signed:32 bit
...
adc_map = OrderedDict([
    ...
    ('pattern',
     RegInfo(
        0x20000062, (0x00000007, 0), False, None, None, False,
        doc="""
        unsigned:3 bit
    ...
    ('pattern_low',
     RegInfo(
        0x20000051, (0x000000FF, 0), False, None, None, False,
        doc="""
        unsigned:8 bit
    ...
    ('gain',
     RegInfo(
        0x20000055, (0x000000F0, 4), False, _from_gain, _to_gain, True,
        doc="""
        unsigned:4 bit
...
channel_map = OrderedDict([
    ...
    ('adc_select',
     RegInfo(
        0x00000800, (0x0000000F, 0), False, _from_onehot, _to_onehot, True,
        doc="""
        unsigned:4 bit
    ...
    ('delay',
     RegInfo(
        0x00000020, (0x000003FF, 0), False, None, None, True,
        doc="""
        unsigned:10 bit
...
baseline_map = OrderedDict([
    ('offset',
     RegInfo(
        0x00000040, (0xFFFFFFFF, 0), False, lambda x: int(np.uint32(x)), None,
        True,
        doc="""
        signed:16 bit
    ...
    ('threshold', RegInfo(
        0x00000100, (), False, np.int32, None, True,
        doc="""
...
cfd_map = OrderedDict([
    ('fraction',
     RegInfo(
        0x00000008, (), False, _from_cf, _to_cf, True,
        doc="""
        unsigned:0.17 bit
...
event_map = OrderedDict([
    ('enable',
     RegInfo(
        0x10000100, (), False, bool, None, False,
        doc="""
        1 bit
    ...
    ('trace_length',
     RegInfo(
         0x00000001, (0x0FFC0000, 18), False, lambda x: int(x*4),
         lambda x: x // 4,
         True,
         doc="""
    ...
    ('slope_threshold',
     RegInfo(
        0x00000004, (), False, None, None, True,
        doc="""
        unsigned:16 bit
